export function registerRuntimeHooks() {
  Hooks.on("renderChatMessageHTML", (message, html) => {
    html.querySelectorAll?.("[data-alternity-damage]").forEach(button => button.addEventListener("click", async () => {
      ...
      for (const token of targets) if (token.actor?.applyDamage) await token.actor.applyDamage(Number(button.dataset.alternityDamage), button.dataset.damageType, { woundHits: Number(button.dataset.woundHits || 1) });
    }));
  });
