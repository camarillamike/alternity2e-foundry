...
export function catalogDocuments() {
  return [
    ...
    ...gear.map(row => ({ ...base(row, row.kind === "drone" ? "gear" : row.kind), system: { ...base(row, row.kind === "drone" ? "gear" : row.kind).system, category: row.category || row.kind, techEra: row.techEra, itemClass: row.class, restriction: row.restriction, mass: row.mass || 0, weaponType: row.type || "", range: row.range || "", speed: row.speed || 0, damage: row.damage || "", damageType: row.damageType || "", special: row.special || [], ammo: weaponAmmoProfile(row), move: row.move || 0, penalty: row.penalty || 0, physical: row.physical || 0, energy: row.energy || 0 } })),
