...
export class AlternityCombat extends Combat {
  ...
  async endCombat() { if (!requireAuthority(canAdvanceWorldClock(), "Only the GM can end an encounter.")) return this; await this.resolveSpecialAmmunition(); return super.endCombat(); }
  ...
  async schedule(combatant, baseCost = 1, { label = "Action", kind = "action", ignoreModifier = false } = {}) {
    ...
    const reactionDelay = flag(combatant, "readied", false) ? 1 : 0, cost = heroAction ? 1 : Math.max(1, Number(baseCost) + Number(modifier?.delay || 0) + statusDelay + reactionDelay), nextTick = scheduleAfter(this.currentTick, cost), sequence = await this.nextSequence();
    const update = { "flags.alternity2e.nextTick": nextTick, "flags.alternity2e.sequence": sequence, "flags.alternity2e.acted": true, "flags.alternity2e.readied": false, "flags.alternity2e.readyTrigger": "", "flags.alternity2e.heroAction": false, "flags.alternity2e.lastAction": `${heroAction ? "Hero Point: " : ""}${label} (${cost} impulse${cost === 1 ? "" : "s"})`, "flags.alternity2e.pendingModifier": modifier ? null : pending };
  ...
  async readyAction(combatant, trigger = "Declared trigger") {
    ...
    const statuses = combatant.actor?.system?.play?.statuses || [], cost = 1 + statusActionDelay(statuses, "ready"), sequence = await this.nextSequence(); await combatant.update({ "flags.alternity2e.nextTick": this.currentTick + cost, "flags.alternity2e.sequence": sequence, "flags.alternity2e.acted": true, "flags.alternity2e.readied": true, "flags.alternity2e.readyTrigger": trigger, "flags.alternity2e.lastAction": `Readied: ${trigger} (${cost} impulse${cost === 1 ? "" : "s"})` }); ui.combat?.render();
  }
  ...
  async previousImpulse() { if (!requireAuthority(canAdvanceWorldClock(), "Only the GM can rewind the encounter clock.")) return this; const position = positionFromTick(Math.max(1, this.currentTick - 1)); await this.update({ round: position.round, turn: null, "flags.alternity2e.impulse": position.impulse }); await this.syncActors(); ui.combat?.render(); }
...
export function registerCombatHooks() {
  ...
  Hooks.on("renderCombatTracker", (app, html) => {
    ...
    controls.innerHTML = `${game.user.isGM ? `<button type="button" data-a2e-prev title="Previous impulse"><i class="fas fa-chevron-left"></i></button>` : ""}<strong>Round ${combat.round || 1} · Impulse ${combat.impulse}/8</strong>${game.user.isGM ? `<button type="button" data-a2e-next title="Next impulse"><i class="fas fa-chevron-right"></i></button><button type="button" data-a2e-skip title="Skip to next scheduled action"><i class="fas fa-forward-step"></i></button>` : ""}`;
